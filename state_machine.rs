...
use crate::page_array::page_array::*;

pub ghost struct ReferenceCounterStateMachine{
    pub page_array: PageArrayGhost,
...
impl ReferenceCounterStateMachine{
    ...
    pub open spec fn page_array_inv(&self) -> bool{
        &&&
        self.page_array.page_array.len() == NUM_PAGES
    }
...
forall|p_id:PageID, pt_id:PagetableID| 
    #![auto] 
    0 <= p_id < NUM_PAGES && self.page_array.page_array[p_id as int].mappings_4k.dom().contains(pt_id) ==>
    ...
    pub open spec fn mapping_match(&self) -> bool{
        ...
        forall|p_id:PageID, pt_id:PagetableID, va:VAddr| 
            #![auto] 
            0 <= p_id < NUM_PAGES && self.page_array.page_array[p_id as int].mappings_4k.dom().contains(pt_id) &&
                self.page_array.page_array[p_id as int].mappings_4k[pt_id].contains(va) ==>
                (
                    ...
                    (
                        ...
                        &&
                        self.page_array.page_array[p_id as int].writing_thread.is_Some()
                        &&
                        self.pagetable_manager.pagetable_dict[pt_id].writing_thread.unwrap() == 
                            self.page_array.page_array[p_id as int].writing_thread.unwrap()
                    )
                )
        &&&
        forall|p_id:PageID, pt_id:PagetableID, va:VAddr| 
...
pub open spec fn write_lock_page_require(old: ReferenceCounterStateMachine, page_id: PageID, thread_id: ThreadID) -> bool{
    &&&
    old.page_array.page_array[page_id as int].writing_thread.is_None()
}
...
pub open spec fn write_lock_page_ensure(old: ReferenceCounterStateMachine, new: ReferenceCounterStateMachine, page_id: PageID, thread_id: ThreadID) -> bool{
    &&&
    old.page_array.page_array[page_id as int].writing_thread == Some(thread_id)
    &&&
    old.pagetable_manager =~= new.pagetable_manager
    &&&
    forall|p_id:PageID| #![auto] 0 <= p_id < NUM_PAGES && page_id != p_id ==> 
        old.page_array.page_array[p_id as int] == new.page_array.page_array[p_id as int]
    &&&
    old.page_array.page_array[page_id as int].mappings_4k == new.page_array.page_array[page_id as int].mappings_4k
}

pub open spec fn write_unlock_page_require(old: ReferenceCounterStateMachine, page_id: PageID, thread_id: ThreadID) -> bool{
    &&&
    old.page_array.page_array[page_id as int].writing_thread.is_Some()
    &&&
    old.page_array.page_array[page_id as int].writing_thread.unwrap() == thread_id
}
...
pub open spec fn add_mapping_aquire(old: ReferenceCounterStateMachine, page_id: PageID, pagetable_id: PagetableID, va:VAddr, thread_id: ThreadID) -> bool{
    ...
    &&&
    old.page_array.page_array[page_id as int].mappings_4k[pagetable_id].contains(va) == false
    &&&
...
pub open spec fn add_mapping_ensure(old: ReferenceCounterStateMachine, new: ReferenceCounterStateMachine, page_id: PageID, pagetable_id: PagetableID, va:VAddr, thread_id: ThreadID) -> bool{
    &&&
    old.page_array.page_array[page_id as int].mappings_4k[pagetable_id].insert(va) =~= new.page_array.page_array[page_id as int].mappings_4k[pagetable_id]
    &&&
    old.page_array.page_array[page_id as int].mappings_4k.dom() =~= new.page_array.page_array[page_id as int].mappings_4k.dom()
    &&&
    ...
    forall|pt_id:PagetableID| #![auto] old.pagetable_manager.pagetable_dict.contains_key(pt_id) && pt_id != pagetable_id ==> 
        ...
        &&
        old.page_array.page_array[page_id as int].mappings_4k[pt_id] =~= new.page_array.page_array[page_id as int].mappings_4k[pt_id]
    &&&
    ...
    &&&
    old.page_array.page_array[page_id as int].writing_thread =~= new.page_array.page_array[page_id as int].writing_thread
    &&&
    old.page_array.page_array.len() =~= new.page_array.page_array.len()
}
...
pub open spec fn remove_pagetable_require(old: ReferenceCounterStateMachine, pagetable_id: PagetableID, thread_id: ThreadID) -> bool{
    ...
    forall|p_id:PageID| #![auto] 0 <= p_id < NUM_PAGES ==> 
        (old.page_array.page_array[p_id as int].mappings_4k.dom().contains(pagetable_id) == false)
        ||
        (old.page_array.page_array[p_id as int].mappings_4k[pagetable_id] =~= Set::<VAddr>::empty())
}
...
pub open spec fn remove_pagetable_ensure(old: ReferenceCounterStateMachine, new: ReferenceCounterStateMachine, pagetable_id: PagetableID, thread_id: ThreadID) -> bool{
    ...
    &&&
    forall|p_id:PageID| #![auto] 0 <= p_id < NUM_PAGES ==> 
        new.page_array.page_array[p_id as int].mappings_4k =~= old.page_array.page_array[p_id as int].mappings_4k.remove(pagetable_id)
    &&&
    forall|p_id:PageID| #![auto] 0 <= p_id < NUM_PAGES ==> 
        old.page_array.page_array[p_id as int].writing_thread =~= new.page_array.page_array[p_id as int].writing_thread
    &&&
        ...
        new.mapping_match(), 
{
    ...
    assert(
        forall|p_id:PageID, pt_id:PagetableID, va:VAddr| 
        #![auto] 
        0 <= p_id < NUM_PAGES && new.page_array.page_array[p_id as int].mappings_4k.dom().contains(pt_id) &&
            new.page_array.page_array[p_id as int].mappings_4k[pt_id].contains(va) ==>
            (
                ...
                (
                    ...
                    &&
                    new.page_array.page_array[p_id as int].writing_thread.is_Some()
                    &&
                    new.pagetable_manager.pagetable_dict[pt_id].writing_thread.unwrap() == 
                    new.page_array.page_array[p_id as int].writing_thread.unwrap()
                )
            )
        );
